...
> 本次直接修复（不在下列待办内）：`NonExistentSqlFileTest` 原先没有任何 @Test 方法（从不执行），
> 且断言的是早已移除的"缺 SQL 文件抛 IllegalStateException"行为，已改写为验证现行"缺文件仅
> 告警跳过"契约的可执行测试；`RdbTestBaseTest` 补上了缺失的 `@EnabledIfDockerInstalled`。

### 可观测性
- `src/io/kudos/test/rdb/SqlTestBase.kt`：fixture 加载日志（含"SQL 文件不存在"的 WARN）全部用
  `println` 手拼时间戳/PID/线程输出，建议换 slf4j logger 并用正确级别（INFO/WARN/ERROR），
  CI 上才能按级别过滤、且不丢 MDC。
- `src/io/kudos/test/rdb/CacheTestResetSupport.kt`：反射找不到类/bean/方法时一律静默返回 null。
  这是有意的降级，但缓存模块方法签名漂移后"缓存重置悄悄失效"将极难排查——建议在 miss 路径
  至少打一条 DEBUG（或首次 miss 打 WARN）。

### 可扩展性
- `SqlTestBase.getTestDataSqlPath()`：`sql/<rdbType>/<ClassSimpleName>.sql` 规则写死，子类只能
  整体 override。建议拆出 `getTestDataSqlRoot()`（默认 "sql"）等更细的 hook，或支持
  `@TestDataSql("path")` 注解式声明。

### 测试覆盖
- `RdbAndRedisCacheTestBase.afterTestDataSetup()` → `CacheTestResetSupport` 的"flushdb + 按
  hash/非 hash 分组 clear/reload"流程没有任何直接测试（`RdbAndRedisCacheTestBaseTest` 只验证
  了属性配置）。反射匹配逻辑（`invoke` 的参数装配、`filterCacheNames`）是纯逻辑，可单测。

### 可维护性
- `SqlTestBase.setUpTestData()`：三段几乎相同的 println 拼接（开始/成功/失败）重复，提取一个
  `logSetup(phase)` 私有函数即可消除；`ResourceDatabasePopulator` 的 separator/comment 配置
  亦可提为常量。
