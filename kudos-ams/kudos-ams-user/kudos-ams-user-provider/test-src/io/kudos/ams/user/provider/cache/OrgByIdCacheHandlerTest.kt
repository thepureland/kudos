package io.kudos.ams.user.provider.cache

import io.kudos.ability.cache.common.kit.CacheKit
import io.kudos.ams.user.common.vo.org.UserOrgCacheItem
import io.kudos.ams.user.provider.dao.UserOrgDao
import io.kudos.ams.user.provider.model.po.UserOrg
import io.kudos.test.container.annotations.EnabledIfDockerInstalled
import io.kudos.test.rdb.RdbAndRedisCacheTestBase
import jakarta.annotation.Resource
import kotlin.test.Test
import kotlin.test.assertEquals
import kotlin.test.assertNotNull
import kotlin.test.assertNull


/**
 * junit test for OrgByIdCacheHandler
 *
 * 测试数据来源：`OrgByIdCacheHandlerTest.sql`
 *
 * @author K
 * @author AI: Cursor
 * @since 1.0.0
 */
@EnabledIfDockerInstalled
class OrgByIdCacheHandlerTest : RdbAndRedisCacheTestBase() {

    @Resource
    private lateinit var cacheHandler: OrgByIdCacheHandler

    @Resource
    private lateinit var userOrgDao: UserOrgDao

    private val newOrgName = "新机构名称"

    @Test
    fun getOrgById() {
        // 存在的
        var id = "38a5e0b2-1111-1111-1111-111111111111"
        val cacheItem2 = cacheHandler.getOrgById(id)
        val cacheItem3 = cacheHandler.getOrgById(id)
        assertNotNull(cacheItem2)
        assert(cacheItem3 === cacheItem2)

        // 不存在的
        id = "no_exist_id"
        assertNull(cacheHandler.getOrgById(id))
    }

    @Test
    fun getOrgsByIds() {
        // 都存在的
        var id1 = "38a5e0b2-1111-1111-1111-111111111111"
        var id2 = "38a5e0b2-2222-2222-2222-222222222222"
        val result2 = cacheHandler.getOrgsByIds(listOf(id1, id2))
        val result3 = cacheHandler.getOrgsByIds(listOf(id1, id2))
        assert(result2.isNotEmpty())
        assert(result3 == result2)

        // 部分存在的
        id1 = "no_exist_id-1"
        var cacheItems = cacheHandler.getOrgsByIds(listOf(id1, id2))
        assertEquals(1, cacheItems.size)

        // 都不存在的
        id2 = "no_exist_id-2"
        cacheItems = cacheHandler.getOrgsByIds(listOf(id1, id2))
        assert(cacheItems.isEmpty())
    }

    @Test
    fun syncOnInsert() {
        // 插入新的记录到数据库
        val id = insertNewRecordToDb()

        // 同步缓存
        cacheHandler.syncOnInsert(id)

        // 验证新记录是否在缓存中
        val cacheItem1 = CacheKit.getValue(cacheHandler.cacheName(), id)
        assertNotNull(cacheItem1)
        val cacheItem2 = cacheHandler.getOrgById(id)
        assertNotNull(cacheItem2)
        val cacheItem3 = cacheHandler.getOrgById(id)
        assert(cacheItem2 === cacheItem3)
    }

    @Test
    fun syncOnUpdate() {
        // 更新数据库中已存在的记录
        val id = "38a5e0b2-2222-2222-2222-222222222222"
        val success = userOrgDao.updateProperties(id, mapOf(UserOrg::name.name to newOrgName))
        assert(success)

        // 同步缓存
        cacheHandler.syncOnUpdate(id)

        // 验证缓存中的记录
        val cacheItem1 = CacheKit.getValue(cacheHandler.cacheName(), id) as UserOrgCacheItem?
        assertNotNull(cacheItem1)
        assertEquals(newOrgName, cacheItem1.name)
        val cacheItem2 = cacheHandler.getOrgById(id)
        assertNotNull(cacheItem2)
        assertEquals(newOrgName, cacheItem2.name)
    }

    @Test
    fun syncOnDelete() {
        // 删除数据库中的记录
        val id = insertNewRecordToDb()
        val deleteSuccess = userOrgDao.deleteById(id)
        assert(deleteSuccess)

        // 同步缓存
        cacheHandler.syncOnDelete(id)

        // 验证缓存中有没有
        val cacheItem1 = CacheKit.getValue(cacheHandler.cacheName(), id)
        assertNull(cacheItem1)
        val cacheItem2 = cacheHandler.getOrgById(id)
        assertNull(cacheItem2)
    }

    @Test
    fun syncOnBatchDelete() {
        // 批量删除数据库中的记录
        val id1 = insertNewRecordToDb()
        val id2 = insertNewRecordToDb()
        val ids = listOf(id1, id2)
        val count = userOrgDao.batchDelete(ids)
        assert(count == 2)

        // 同步缓存
        cacheHandler.syncOnBatchDelete(ids)

        // 验证缓存中有没有
        var cacheItem = CacheKit.getValue(cacheHandler.cacheName(), id1)
        assertNull(cacheItem)
        cacheItem = cacheHandler.getOrgById(id1)
        assertNull(cacheItem)
        cacheItem = CacheKit.getValue(cacheHandler.cacheName(), id2)
        assertNull(cacheItem)
        cacheItem = cacheHandler.getOrgById(id2)
        assertNull(cacheItem)
    }

    private fun insertNewRecordToDb(): String {
        val userOrg = UserOrg().apply {
            name = "测试机构_${System.currentTimeMillis()}"
            tenantId = "38a5e0b2-1111-1111-1111-111111111111"
            orgTypeDictCode = "ORG_TYPE_DEFAULT"
            active = true
        }
        return userOrgDao.insert(userOrg)
    }

}
