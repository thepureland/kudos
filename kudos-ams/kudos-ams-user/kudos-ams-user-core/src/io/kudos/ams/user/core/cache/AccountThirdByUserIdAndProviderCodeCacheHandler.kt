...
import io.kudos.ams.user.common.vo.user.UserAccountThirdCacheItem
import io.kudos.ams.user.common.vo.user.UserAccountThirdSearchPayload
import io.kudos.ams.user.core.dao.UserAccountThirdDao
import io.kudos.ams.user.core.model.po.UserAccountThird
...
@Component
//region your codes 1
open class AccountThirdByUserIdAndProviderCodeCacheHandler : AbstractCacheHandler<UserAccountThirdCacheItem>() {
//endregion your codes 1
    ...
    override fun doReload(key: String): UserAccountThirdCacheItem? {
        ...
        return getSelf<AccountThirdByUserIdAndProviderCodeCacheHandler>().getAccountThird(
            parts[0], parts[1]
        )
    }
    ...
    open fun syncOnInsert(any: Any, id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<AccountThirdByUserIdAndProviderCodeCacheHandler>().getAccountThird(userId, providerCode)
            }
    ...
    open fun syncOnUpdateActive(id: String, active: Boolean) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (active && CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<AccountThirdByUserIdAndProviderCodeCacheHandler>().getAccountThird(
