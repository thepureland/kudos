...
import io.kudos.ams.user.common.vo.contact.UserContactWayCacheItem
import io.kudos.ams.user.common.vo.contact.UserContactWaySearchPayload
import io.kudos.ams.user.core.dao.UserContactWayDao
import io.kudos.ams.user.core.model.po.UserContactWay
...
@Component
//region your codes 1
open class UserContactWayByUserIdCacheHandler : AbstractCacheHandler<List<UserContactWayCacheItem>>() {
//endregion your codes 1
    ...
    override fun doReload(key: String): List<UserContactWayCacheItem>? {
        return getSelf<UserContactWayByUserIdCacheHandler>().getContactWays(key)
    }
    ...
    open fun syncOnInsert(any: Any, id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<UserContactWayByUserIdCacheHandler>().getContactWays(userId)
            }
    ...
    open fun syncOnUpdateActive(id: String, active: Boolean) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (active && CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<UserContactWayByUserIdCacheHandler>().getContactWays(contactWay.userId!!)
            }
            log.debug("缓存同步完成。")
        }
    }
