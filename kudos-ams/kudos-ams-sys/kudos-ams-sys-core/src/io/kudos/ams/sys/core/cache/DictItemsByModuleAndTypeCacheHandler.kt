...
import io.kudos.ams.sys.core.dao.SysDictItemDao
import io.kudos.ams.sys.core.model.po.SysDictItem
...
open class DictItemsByModuleAndTypeCacheHandler : AbstractCacheHandler<List<SysDictItemCacheItem>>() {
    ...
    override fun doReload(key: String): List<SysDictItemCacheItem> {
        ...
        return getSelf<DictItemsByModuleAndTypeCacheHandler>().getDictItems(
    ...
    open fun syncOnInsert(any: Any, id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            val dict = dictByIdCacheHandler.getDictById(dictId)
            if (dict == null) {
                log.error("缓存${dictByIdCacheHandler.cacheName()}中找不到id为${id}的字典！")
                return
            }
    ...
    open fun syncOnUpdateActive(id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            val dict = dictByIdCacheHandler.getDictById(dictIds.first() as String)
            if (dict == null) {
                log.error("缓存${dictByIdCacheHandler.cacheName()}中找不到id为${id}的字典！")
                return
            }
