 ...
 */
@Component
open class TenantIdsBySystemCodeCacheHandler : AbstractCacheHandler<List<String>>() {
    ...
    override fun doReload(key: String): List<String> = getSelf<TenantIdsBySystemCodeCacheHandler>().getTenantIds(key)
    ...
    open fun syncOnInsert(any: Any, id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<TenantIdsBySystemCodeCacheHandler>().getTenantIds(systemCode) // 重新缓存
            }
