 ...
 */
@Component
open class ResourceIdsByTenantIdAndGroupCodeCacheHandler : AbstractCacheHandler<List<String>>() {

    @Autowired
    private lateinit var groupIdByTenantIdAndGroupCodeCacheHandler: GroupIdByTenantIdAndGroupCodeCacheHandler

    @Autowired
    private lateinit var authGroupDao: io.kudos.ams.auth.core.dao.AuthGroupDao
    ...
    override fun doReload(key: String): List<String> {
        ...
        return getSelf<ResourceIdsByTenantIdAndGroupCodeCacheHandler>().getResourceIds(
    ...
    open fun getResourceIds(tenantId: String, groupCode: String): List<String> {
        ...
        // 1. 从缓存中获取用户组ID
        val groupId = groupIdByTenantIdAndGroupCodeCacheHandler.getGroupId(tenantId, groupCode)
        if (groupId == null) {
    ...
    open fun syncOnGroupRoleInsert(tenantId: String, groupCode: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<ResourceIdsByTenantIdAndGroupCodeCacheHandler>().getResourceIds(tenantId, groupCode)
            }
    ...
    open fun syncOnGroupDelete(tenantId: String, groupCode: String) {
        ...
        groupIdByTenantIdAndGroupCodeCacheHandler.evict(groupIdByTenantIdAndGroupCodeCacheHandler.getKey(tenantId, groupCode))
    }
