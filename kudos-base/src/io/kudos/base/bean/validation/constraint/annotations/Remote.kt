...
/**
 * Remote constraint annotation. The client issues a validation request to the server, where the user may implement
 * arbitrary validation logic. Property-level annotation.
 *
 * @author K
 * @since 1.0.0
 */
...
annotation class Remote(
    /**
     * The class that performs the logical validation.
     */
    val checkClass: KClass<out IBeanValidator<*>>,
    /**
     * The validation request URL.
     */
     ...
     */
    val message: String = "{io.kudos.base.bean.validation.constraint.annotations.Remote.message}",
    /**
    ...
    /**
     * Multiple Remote constraints.
     * When this annotation is used, Bean Validation processes each element of the array as an ordinary constraint
     * annotation and validates it; validation passes only when all constraints are satisfied.
     */
    @MustBeDocumented
    @Retention(AnnotationRetention.RUNTIME)
