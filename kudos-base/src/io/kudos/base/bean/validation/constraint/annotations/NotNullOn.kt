...
/**
 * Conditional non-null constraint annotation; property-level annotation.
 *
 *
 * Whether the value of the current property may be null depends on the defined expression. If the expression is false, the property value may be null (i.e., it is optional); if the expression is true, the property value is required.
 * Note: this constraint annotation cannot be used together with @NotNull.
 * Note: equivalent NotEmptyOn / NotBlankOn constraint annotations cannot be implemented in the same way. NotNullOn works because every constraint annotation returns true during validation when the value is null.
 *
 * @author K
 * @since 1.0.0
 */
...
annotation class NotNullOn(
    /**
     * The precondition this NotNull constraint depends on. When the condition holds, this is equivalent to @NotNull; when it does not hold, it is equivalent to having no such constraint annotation.
     */
    val depends: Depends,
    /**
