...
/**
 * Comparison constraint annotation, a property-level annotation. The values of the two properties must satisfy the specified logical condition; a precondition for the comparison can be specified.
 *
 *
 * Used to constrain the logical relationship between two property values; the two properties must be of the same type and both must implement the Comparable interface.
 * Properties of type Array<*> are supported, but the two arrays must be the same size, and every element of the arrays must implement the Comparable interface;
 * in that case, the values at the same index in the two arrays are compared, and the constraint is only fully satisfied if every pair satisfies the constraint.
 * Properties do not support IntArray, CharArray and similar types.
 *
 * @author K
 * @since 1.0.0
 */
...
annotation class Compare(
    /**
     * The second property to compare (right operand).
     */
    val anotherProperty: String,
    /**
     * The comparison logic.
     */
    val logic: LogicOperatorEnum = LogicOperatorEnum.EQ,
    /**
     * Precondition that the comparison depends on; the comparison is only performed when the precondition holds.
     */
    val depends: Depends = Depends(properties = []),
    /**
     ...
     */
    val message: String = "{io.kudos.base.bean.validation.constraint.annotations.Compare.message}",
    /**
    ...
    /**
     * Multiple Compare constraints.
     * When this constraint annotation is used, Bean Validation processes every element in the Compare array as a regular constraint annotation and validates it; validation only passes when every constraint is satisfied.
     */
    @MustBeDocumented
    @Retention(AnnotationRetention.RUNTIME)
