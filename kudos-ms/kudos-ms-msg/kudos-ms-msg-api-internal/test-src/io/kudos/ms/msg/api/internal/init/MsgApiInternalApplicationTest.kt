...
/**
 * Pure unit test for [MsgApiInternalApplication] and its [main] entry point.
 *
 ...
 * @since 1.0.0
 */
internal class MsgApiInternalApplicationTest {

    @Test
    fun canBeInstantiated() {
        assertNotNull(MsgApiInternalApplication())
    }
    ...
    fun main_delegatesToSpringApplicationRun_withSourceClassAndArgs() {
        ...
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(MsgApiInternalApplication::class.java, "--server.port=0", "extra")
            }
    ...
    fun main_forwardsEmptyArgs() {
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(MsgApiInternalApplication::class.java)
            }
            mocked.verifyNoMoreInteractions()
        }
    }
