...
/**
 * Pure unit test for [MsgApiPublicApplication] and its [main] entry point.
 *
 ...
 * @since 1.0.0
 */
internal class MsgApiPublicApplicationTest {

    @Test
    fun classIsInstantiable() {
        assertNotNull(MsgApiPublicApplication())
    }
    ...
    fun main_delegatesToSpringApplicationRun_withSourceClassAndArgs() {
        ...
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(MsgApiPublicApplication::class.java, "--server.port=0", "extra")
            }
    ...
    fun main_forwardsEmptyArgs() {
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(MsgApiPublicApplication::class.java)
            }
            mocked.verifyNoMoreInteractions()
        }
    }
