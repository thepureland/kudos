...
/**
 * Pure unit test for [MsgApiWebAutoConfiguration] — verifies the component-name contract
 * without booting any Spring container.
 *
 * @author K
 * @since 1.0.0
 */
internal class MsgApiWebAutoConfigurationTest {

    @Test
    fun getComponentName_returnsModuleName() {
        assertEquals("kudos-ms-msg-api-public", MsgApiWebAutoConfiguration().getComponentName())
    }

    @Test
    fun getComponentName_isStableAcrossCalls() {
        val config = MsgApiWebAutoConfiguration()
    ...
    fun getComponentName_isStableAcrossInstances() {
        assertEquals(
            MsgApiWebAutoConfiguration().getComponentName(),
            MsgApiWebAutoConfiguration().getComponentName()
        )
    }
    ...
    fun isComponentInitializer() {
        ...
        assertTrue(
            IComponentInitializer::class.java.isAssignableFrom(MsgApiWebAutoConfiguration::class.java)
        )
    }
}
