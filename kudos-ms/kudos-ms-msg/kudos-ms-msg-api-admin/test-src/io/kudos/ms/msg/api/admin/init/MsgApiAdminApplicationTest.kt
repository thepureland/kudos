...
/**
 * Pure unit test for [MsgApiAdminApplication] and its [main] entry point.
 *
 ...
 * @since 1.0.0
 */
internal class MsgApiAdminApplicationTest {

    @Test
    fun classIsInstantiable() {
        assertNotNull(MsgApiAdminApplication())
    }
    ...
    fun main_delegatesToSpringApplicationRun_withSourceClassAndArgs() {
        ...
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(MsgApiAdminApplication::class.java, "--server.port=0", "extra")
            }
    ...
    fun main_forwardsEmptyArgs() {
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(MsgApiAdminApplication::class.java)
            }
            mocked.verifyNoMoreInteractions()
        }
    }
