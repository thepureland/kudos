 ...
 */
@Component
open class DataSourceByTenantIdAnd3CodesCache : AbstractCacheHandler<SysDataSourceCacheItem>() {
    ...
    override fun doReload(key: String): SysDataSourceCacheItem? {
        ...
        return getSelf<DataSourceByTenantIdAnd3CodesCache>().getDataSource(
        ...
        if (CacheKit.isCacheActive(CACHE_NAME) && CacheKit.isWriteInTime(CACHE_NAME)) {
            ...
            if (!tenantId.isNullOrBlank()) {
                ...
                if (active == null || active) {
                    ...
                    // 缓存
                    getSelf<DataSourceByTenantIdAnd3CodesCache>().getDataSource(
    ...
    open fun syncOnUpdateActive(id: String, active: Boolean) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            val ds = dataSourceByIdCache.getDataSourceById(id)
            if (ds == null) {
                log.error("缓存${dataSourceByIdCache.cacheName()}中找不到id为${id}的数据源！")
                return
            }
    ...
    open fun syncOnDelete(id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            val ds = dataSourceByIdCache.getDataSourceById(id)
            if (ds == null) {
                log.error("缓存${dataSourceByIdCache.cacheName()}中找不到id为${id}的对象！")
                return
            }
