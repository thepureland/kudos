...
/**
 * Pure unit test for [SysApiWebAutoConfiguration] — verifies the component-name contract
 * without booting any Spring container.
 *
 * @author K
 * @since 1.0.0
 */
internal class SysApiWebAutoConfigurationTest {

    @Test
    fun getComponentName_returnsModuleName() {
        assertEquals("kudos-ms-sys-api-public", SysApiWebAutoConfiguration().getComponentName())
    }

    @Test
    fun getComponentName_isStableAcrossCalls() {
        val config = SysApiWebAutoConfiguration()
    ...
    fun getComponentName_isStableAcrossInstances() {
        assertEquals(
            SysApiWebAutoConfiguration().getComponentName(),
            SysApiWebAutoConfiguration().getComponentName()
        )
    }
    ...
    fun isComponentInitializer() {
        ...
        assertTrue(
            IComponentInitializer::class.java.isAssignableFrom(SysApiWebAutoConfiguration::class.java)
        )
    }
}
