...
/**
 * Pure unit test for [SysApiPublicApplication] and its [main] entry point.
 *
 ...
 * @since 1.0.0
 */
internal class SysApiPublicApplicationTest {

    @Test
    fun classIsInstantiable() {
        assertNotNull(SysApiPublicApplication())
    }
    ...
    fun main_delegatesToSpringApplicationRun_withSourceClassAndArgs() {
        ...
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(SysApiPublicApplication::class.java, "--server.port=0", "extra")
            }
    ...
    fun main_forwardsEmptyArgs() {
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(SysApiPublicApplication::class.java)
            }
            mocked.verifyNoMoreInteractions()
        }
    }
