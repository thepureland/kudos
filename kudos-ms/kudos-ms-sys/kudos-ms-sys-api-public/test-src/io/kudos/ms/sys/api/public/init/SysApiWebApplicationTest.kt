...
/**
 * Pure unit test for [SysApiWebApplication] and its [main] entry point.
 *
 ...
 * @since 1.0.0
 */
internal class SysApiWebApplicationTest {

    @Test
    fun classIsInstantiable() {
        assertNotNull(SysApiWebApplication())
    }
    ...
    fun main_delegatesToSpringApplicationRun_withSourceClassAndArgs() {
        ...
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(SysApiWebApplication::class.java, "--server.port=0", "extra")
            }
    ...
    fun main_forwardsEmptyArgs() {
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(SysApiWebApplication::class.java)
            }
            mocked.verifyNoMoreInteractions()
        }
    }
