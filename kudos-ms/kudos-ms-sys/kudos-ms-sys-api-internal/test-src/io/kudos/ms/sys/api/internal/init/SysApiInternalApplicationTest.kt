...
/**
 * Pure unit test for [SysApiInternalApplication] and its [main] entry point.
 *
 ...
 * @since 1.0.0
 */
internal class SysApiInternalApplicationTest {

    @Test
    fun classIsInstantiable() {
        assertNotNull(SysApiInternalApplication())
    }
    ...
    fun main_delegatesToSpringApplicationRun_withSourceClassAndArgs() {
        ...
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(SysApiInternalApplication::class.java, "--server.port=0", "extra")
            }
    ...
    fun main_forwardsEmptyArgs() {
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(SysApiInternalApplication::class.java)
            }
            mocked.verifyNoMoreInteractions()
        }
    }
    ...
    fun main_forwardsUnicodeArgVerbatim() {
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(SysApiInternalApplication::class.java, "--app.name=系统-内部")
            }
            mocked.verifyNoMoreInteractions()
        }
    }
