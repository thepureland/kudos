...
create unique index if not exists "uq_sys_datasource"
    on "sys_datasource" ("name", "micro_service_code", "atomic_service_code", "tenant_id");

comment on table "sys_datasource" is '数据源';
comment on column "sys_datasource"."id" is '主键';
comment on column "sys_datasource"."name" is '名称';
comment on column "sys_datasource"."micro_service_code" is '微服务编码';
comment on column "sys_datasource"."atomic_service_code" is '原子服务编码';
