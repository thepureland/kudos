...
/**
 * Pure unit test for [UserApiInternalApplication] and its [main] entry point.
 *
 ...
 * @since 1.0.0
 */
internal class UserApiInternalApplicationTest {

    @Test
    fun classIsInstantiable() {
        assertNotNull(UserApiInternalApplication())
    }
    ...
    fun main_delegatesToSpringApplicationRun_withSourceClassAndArgs() {
        ...
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(UserApiInternalApplication::class.java, "--server.port=0", "extra")
            }
    ...
    fun main_forwardsEmptyArgs() {
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            ...
            mocked.verify {
                SpringApplication.run(UserApiInternalApplication::class.java)
            }
            mocked.verifyNoMoreInteractions()
        }
    }
