 ...
 */
@Component
open class UserIdByTenantIdAndUsernameCache : AbstractKeyValueCacheHandler<String>() {
    ...
    override fun doReload(key: String): String? {
        ...
        return getSelf<UserIdByTenantIdAndUsernameCache>().getUserId(
    ...
    open fun syncOnUpdate(any: Any?, id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<UserIdByTenantIdAndUsernameCache>().getUserId(user.tenantId, user.username) // 重新缓存
            }
            log.debug("${CACHE_NAME}缓存同步完成。")
        }
    }
