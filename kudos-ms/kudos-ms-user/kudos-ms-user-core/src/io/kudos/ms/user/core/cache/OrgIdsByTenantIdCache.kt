 ...
 */
@Component
open class OrgIdsByTenantIdCache : AbstractKeyValueCacheHandler<List<String>>() {
    ...
    override fun doReload(key: String): List<String> = getSelf<OrgIdsByTenantIdCache>().getOrgIds(key)
    ...
    open fun syncOnInsert(any: Any, id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<OrgIdsByTenantIdCache>().getOrgIds(tenantId) // 重新缓存
            }
    ...
    open fun syncOnUpdateActive(id: String, active: Boolean) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<OrgIdsByTenantIdCache>().getOrgIds(userOrg.tenantId) // 重新缓存
            }
            log.debug("缓存同步完成。")
        }
    }
