 ...
 */
@Component
open class UserIdsByOrgIdCacheHandler : AbstractCacheHandler<List<String>>() {
    ...
    override fun doReload(key: String): List<String> = getSelf<UserIdsByOrgIdCacheHandler>().getUserIds(key)
    ...
    open fun syncOnInsert(any: Any, id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<UserIdsByOrgIdCacheHandler>().getUserIds(orgId) // 重新缓存
            }
    ...
    open fun syncOnOrgUserChange(orgId: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            evict(orgId)
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<UserIdsByOrgIdCacheHandler>().getUserIds(orgId)
            }
            log.debug("${CACHE_NAME}缓存同步完成。")
        }
    }
