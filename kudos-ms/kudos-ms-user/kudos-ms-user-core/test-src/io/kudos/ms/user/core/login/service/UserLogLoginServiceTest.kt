 ...
 *
 * Test data source: `UserLogLoginServiceTest.sql`
 *
 * @author K
 * @author AI: Cursor
 * @since 1.0.0
...
class UserLogLoginServiceTest : RdbAndRedisCacheTestBase() {
    ...
    fun getLoginsByUserId() {
        ...
        // Verify descending order by time
        if (logins.size > 1) {
    ...
    fun countSuccessLogins() {
        ...
        assertTrue(count >= 4) // At least 4 successful records

        // Test with specified user
        val userId = "1a14e2ae-0000-0000-0000-000000000078"
        val userCount = userLogLoginService.countSuccessLogins(tenantId, userId, null, null)
        assertTrue(userCount >= 3) // User 1 has at least 3 successful records
    }
    ...
    fun countFailureLogins() {
        ...
        assertTrue(count >= 2) // At least 2 failed records

        // Test with specified user
        val userId = "1a14e2ae-0000-0000-0000-000000000078"
        val userCount = userLogLoginService.countFailureLogins(tenantId, userId, null, null)
        assertTrue(userCount >= 1) // User 1 has at least 1 failed record
    }
}