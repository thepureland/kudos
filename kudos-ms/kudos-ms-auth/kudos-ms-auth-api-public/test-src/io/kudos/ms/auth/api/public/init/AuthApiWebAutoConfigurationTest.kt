...
/**
 * Pure unit test for [AuthApiWebAutoConfiguration] — verifies the component-name contract and the
 * Spring stereotype/scan annotations without booting any Spring container.
 *
 * @author K
 * @since 1.0.0
 */
internal class AuthApiWebAutoConfigurationTest {

    @Test
    fun getComponentName_returnsModuleName() {
        assertEquals("kudos-ms-auth-api-public", AuthApiWebAutoConfiguration().getComponentName())
    }

    @Test
    fun getComponentName_isStableAcrossCalls() {
        val config = AuthApiWebAutoConfiguration()
    ...
    fun getComponentName_isStableAcrossInstances() {
        assertEquals(
            AuthApiWebAutoConfiguration().getComponentName(),
            AuthApiWebAutoConfiguration().getComponentName()
        )
    }
    ...
    fun isComponentInitializer() {
        ...
        assertTrue(
            IComponentInitializer::class.java.isAssignableFrom(AuthApiWebAutoConfiguration::class.java)
        )
    }
    ...
    fun carriesConfigurationAnnotation() {
        assertNotNull(
            AuthApiWebAutoConfiguration::class.java.getAnnotation(Configuration::class.java),
    ...
    fun carriesComponentScanForOwnBasePackage() {
        val scan = AuthApiWebAutoConfiguration::class.java.getAnnotation(ComponentScan::class.java)
