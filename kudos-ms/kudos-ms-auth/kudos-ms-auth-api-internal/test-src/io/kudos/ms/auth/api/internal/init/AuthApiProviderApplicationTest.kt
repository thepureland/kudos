...
/**
 * Pure unit test for [AuthApiProviderApplication] and its [main] entry point.
 *
 ...
 * @since 1.0.0
 */
internal class AuthApiProviderApplicationTest {

    @Test
    fun canBeInstantiated() {
        assertNotNull(AuthApiProviderApplication())
    }
    ...
    fun main_delegatesToSpringApplicationRun_withSourceClassAndArgs() {
        ...
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            main(args)
            mocked.verify {
                SpringApplication.run(AuthApiProviderApplication::class.java, "--server.port=0", "extra")
            }
    ...
    fun main_forwardsEmptyArgs() {
        Mockito.mockStatic(SpringApplication::class.java).use { mocked ->
            main(emptyArray())
            mocked.verify {
                SpringApplication.run(AuthApiProviderApplication::class.java)
            }
            mocked.verifyNoMoreInteractions()
        }
    }
