...
internal class AuthRoleDataScopeServiceUnitTest {
    ...
    private val dao = mock(AuthRoleOrgDao::class.java)
    ...
    private val service = AuthRoleDataScopeService(dao).apply {
        ...
        inject("userAccountHashCache", userAccountHashCache)
        inject("userOrgDao", userOrgDao)
    ...
    fun bindOrgs_trimsAndFiltersBlanks() {
        whenCalled(dao.deleteByRoleId("role1")).thenReturn(0)
    ...
    fun bindOrgs_allBlank_clearsAndReturnsZero() {
        whenCalled(dao.deleteByRoleId("role1")).thenReturn(3)
    ...
    fun getOrgIdsByRoleId_delegatesToDao() {
        whenCalled(dao.searchOrgIdsByRoleId("role1")).thenReturn(setOf("o1", "o2"))
        assertEquals(setOf("o1", "o2"), service.getOrgIdsByRoleId("role1"))
        verify(dao).searchOrgIdsByRoleId("role1")
    }
    ...
    fun resolve_userWithoutRoles_selfOnly() {
        ...
        val vo = service.resolveUserDataScope("u-none")
        assertFalse(vo.all)
        assertTrue(vo.self)
    ...
    fun resolve_anyAllScope_shortCircuitsToAll() {
        ...
        val vo = service.resolveUserDataScope("u1")
        assertTrue(vo.all)
        // ALL short-circuits before any org lookup.
        verify(userOrgDao, never()).searchOrgAndDescendantIds(anyString())
    }
    ...
    fun resolve_nullDataScopeTreatedAsAll() {
        ...
        assertTrue(service.resolveUserDataScope("u2").all)
    }
    ...
    fun resolve_customWithNoGrants_andNoOther_fallsBackToSelfOnly() {
        ...
        whenCalled(userAccountHashCache.getUsersByIds(anyStringCollection())).thenReturn(emptyMap())
        whenCalled(dao.searchOrgIdsByRoleId("rCustom")).thenReturn(emptySet())
        val vo = service.resolveUserDataScope("u3")
        assertFalse(vo.all)
        assertTrue(vo.self)
        assertTrue(vo.orgIds.isEmpty())
    }
    ...
    fun resolve_selfScope_setsSelfFlag() {
        ...
        whenCalled(userAccountHashCache.getUsersByIds(anyStringCollection())).thenReturn(emptyMap())
        val vo = service.resolveUserDataScope("u4")
        assertFalse(vo.all)
        assertTrue(vo.self)
        assertTrue(vo.orgIds.isEmpty())
    }
