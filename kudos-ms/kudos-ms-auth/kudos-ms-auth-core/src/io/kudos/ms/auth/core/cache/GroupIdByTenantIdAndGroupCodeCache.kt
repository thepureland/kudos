 ...
 */
@Component
open class GroupIdByTenantIdAndGroupCodeCache : AbstractKeyValueCacheHandler<String>() {
    ...
    override fun doReload(key: String): String? {
        ...
        return getSelf<GroupIdByTenantIdAndGroupCodeCache>().getGroupId(
    ...
    open fun syncOnUpdate(any: Any?, id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<GroupIdByTenantIdAndGroupCodeCache>().getGroupId(group.tenantId, group.code) // 重新缓存
            }
            log.debug("${CACHE_NAME}缓存同步完成。")
        }
    }
