 ...
 */
@Component
open class RoleIdByTenantIdAndRoleCodeCacheHandler : AbstractCacheHandler<String>() {
    ...
    override fun doReload(key: String): String? {
        ...
        return getSelf<RoleIdByTenantIdAndRoleCodeCacheHandler>().getRoleId(
    ...
    open fun syncOnUpdate(any: Any?, id: String) {
        if (CacheKit.isCacheActive(CACHE_NAME)) {
            ...
            if (CacheKit.isWriteInTime(CACHE_NAME)) {
                getSelf<RoleIdByTenantIdAndRoleCodeCacheHandler>().getRoleId(role.tenantId, role.code) // 重新缓存
            }
            log.debug("${CACHE_NAME}缓存同步完成。")
        }
    }
