...
ALTER TABLE "auth_security_event" ADD CONSTRAINT IF NOT EXISTS "ck_auth_security_event_workflow_version"
    CHECK ("workflow_version" >= 0);

ALTER TABLE "auth_security_event" ADD CONSTRAINT IF NOT EXISTS "ck_auth_security_event_status"
    CHECK ("status" IN ('OPEN', 'ACKNOWLEDGED', 'CLOSED'));

ALTER TABLE "auth_security_event" ADD CONSTRAINT IF NOT EXISTS "ck_auth_security_event_workflow_fields"
    CHECK (
        (
            "status" = 'OPEN'
