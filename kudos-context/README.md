...
- ❗ **`FailedDataRetryScanner.scheduleAll` 用 `KudosContextHolder.get()`**：在 `@PostConstruct` 线程里 **强制创建** 一个新 `KudosContext`，仅为取 `atomicServiceCode`——通常此时 `atomicServiceCode == null`，所有 handler 的锁 key 都退化为 `failed-data-retry-${businessType}_null`。多原子服务部署到同一进程时锁 key 会跨服务冲突。
...
| `retry` | `RetryConfigTest`、`FailedDataRetryScannerTest` | ❌ `AbstractFailedDataHandler.persistFailedData` 与 `readDataFromFile` 序列化往返无测试；`scheduleAll` 与 cron 触发无测试 |
