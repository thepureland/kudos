...
启动期：ComponentInitializationDispatcher 作为 BeanDefinitionRegistryPostProcessor + SmartInstantiationAwareBeanPostProcessor：
  ...
  ② setBeanFactory
      └─ 对每个 IComponentInitializer compName：
          收集 factoryBeanName == compName 的所有 child bean + compName 自身 → remainingBeanNames[compName] = set
  ...
  ③ Bean 创建循环
      ...
      │       └─ 对所有 remainingBeanNames：remainingBeanNames[comp].remove(beanName)（幂等）
      │            if 集合变空: initializerInstances[comp].afterInit()
      │
      └─ getEarlyBeanReference(bean) (循环依赖/CGLIB 早期暴露)
              └─ 当前实现转发到 postProcessAfterInitialization —— 让代理也参与完成度统计
                 统计基于"剩余 bean 集合移除"（幂等），同一 beanName 两条路径都走过也只记一次
```
...
5. **`ComponentInitializationDispatcher.getEarlyBeanReference`**：~~循环依赖 / CGLIB 代理场景下可能造成 `remainingCount` 双扣减、`afterInit` 触发时机偏移~~ 已修复——完成度统计改为"剩余 bean 集合移除"（`Set.remove` 幂等），同一 beanName 经 early-reference 与正常初始化两条路径进入也只计一次。集成测试覆盖仍待补。
...
- ❗ **`LockTool` fallback 是本地 `NormalLockService`**：[`LockTool.LOCK_SERVICE`](src/io/kudos/context/lock/LockTool.kt) `by lazy` 时若 Spring 容器里没有任何 `ILockProvider` Bean，会 new 一个 `NormalLockService()`（首次解析时已输出 **WARN 日志**提示分布式锁失效）。结果仍是**多实例部署下分布式锁失效**——所有节点都用各自的内存锁。生产环境必须明确依赖 `kudos-ability-distributed-redisson` 等模块注册 Redisson 锁。仍**没有启动期硬断言**（fail-fast 选项），只有日志提示。
...
- ✅ **`YamlPropertySourceFactory.SOURCE_MAP` 已改为 `Collections.synchronizedMap(LinkedHashMap)`**：消除"运行时动态 reload propertySource 扩展"场景下的潜在竞态（不能用 `ConcurrentHashMap`——sourceName 可为 null）。内部读 API 仍包 `Collections.unmodifiableMap`，`allSourcePath()` 迭代已按 synchronizedMap 契约加锁。
...
- ✅ **`ComponentInitializationDispatcher` 双扣减问题已修复**：完成度统计从"child 计数扣减"改为"剩余 bean 集合移除"（幂等），`getEarlyBeanReference` 与 `postProcessAfterInitialization` 重叠进入同一 beanName 只计一次（见 [已知约束 5](#已知约束与陷阱)），**集成测试覆盖仍待补**。
...
| `config` | `OrderPropertiesTest`（插入序保持、更新/删除/清空） | ❌ `YamlPropertySourceFactory` Nacos / 本地合并未测试；`OrderProperties.putIfAbsent` / 两参 `remove` 同步逻辑（本轮新增）未测试 |
...
> 本轮已直接修复（供 review 对照）：`OrderProperties` 补 `putIfAbsent` / 两参 `remove` 覆写同步 `keyList`；`NormalLockService.tryLock` 的 `sec * 1000` Int 溢出改 `1000L`；`AbstractFailedDataHandler` 泛型反射结果缓存为 `lazy`；`FailedDataRetryScanner` 600s 提取为命名常量、锁竞争 warn 日志带上锁 key；`LockTool` fallback 本地锁时输出 WARN；`TransactionTool` 日志门面从 commons-logging 统一为 kudos `LogFactory`；`YamlPropertySourceFactory.SOURCE_MAP` 改 `synchronizedMap`；`ComponentInitializationDispatcher` 双扣减 bug 改为幂等的集合移除；`IFailedDataHandler.persistFailedData` KDoc 修正失败契约描述。以下为**不宜本轮直接改**的发现。

### 1. Kotlin 惯用法

- `src/io/kudos/context/core/ClientInfo.kt`：Java 风格 Builder 约 100 行模板代码，Kotlin 下用"具名参数 + 默认值"的构造器即可全部替代，且字段目前是可变 `var`（构造后可被改）。涉及 public API 变更，留待大版本。
- `src/io/kudos/context/kit/SpringKit.kt`：`getBeanOrNull(beanClass)` 用 `getBeanNamesForType().isNotEmpty() + getBean()` 两步，同类型多个 bean 时仍会抛 `NoUniqueBeanDefinitionException` 而非返回 null。可改 `ctx.getBeanProvider(clazz.java).getIfUnique()`，但"多 bean 时返回 null 还是抛异常"是语义变化，需先确认调用方期望。

### 2. 功能缺陷 / 补充

- `src/io/kudos/context/core/KudosContext.kt`：缺少 `copy()`/快照方法。跨线程池显式传递上下文（README 已知约束 2 的官方建议做法）目前只能手工逐字段拷贝，建议提供浅拷贝 + map 字段防御性复制的 `copy()`，并与 `KudosContextElement` 协程入口配合。
- `src/io/kudos/context/core/KudosContextElement.kt` 与 `KudosContextHolder`：两套存储互不感知，桥接责任全在入口插件。建议补 `withKudosContextFromThreadLocal { }` 之类的官方桥接 helper，减少每个 web 模块重复实现。
- `src/io/kudos/context/retry/AbstractFailedDataHandler.kt`：无最大重试次数 / 死信目录。永远处理失败的文件会按 cron 无限重试，建议支持 `maxRetryTimes` + 超限移入 `dead/` 子目录并告警。
- `src/io/kudos/context/retry/FailedDataRetryScanner.kt`：600s 锁租约已提取常量但仍写死，建议暴露为 `IFailedDataHandler` 属性或系统属性 `kudos.retry.lock-lease-seconds`。

### 3. 安全性

- `src/io/kudos/context/retry/AbstractFailedDataHandler.kt` / `IFailedDataHandler.businessType`：`businessType` 直接拼入文件路径（`{base}/{businessType}/...`）。若实现方把外部输入用作 businessType 存在路径穿越风险，建议持久化前校验白名单字符（如 `[A-Za-z0-9_-]+`）。
- `src/io/kudos/context/retry/RetryConfig.kt`：默认落盘目录为 `java.io.tmpdir`（多数系统全局可读写），失败数据是明文 JSON、可能含业务敏感字段。建议文档明确生产环境应配置专有目录并收紧权限，或落盘时设置文件权限（POSIX `rw-------`）。
- `src/io/kudos/context/kit/SpringKit.kt`：`applicationContext` 是公开可写 `var`（且接受 null），任意代码可替换/置空全局容器引用导致整个模块失效。建议改为 `internal set` 或仅允许 `SpringContextInitializer` 写入（破坏现有测试写法，需协调）。

### 4. 测试覆盖（对照 src 与 test-src）

- `KudosContextElement.kt` 全套协程传播（`currentKudosContext` / `withKudosContext*` / `launchWithKudos`）无任何测试——纯 kotlinx-coroutines 逻辑，可用 `runBlocking` 直接测。
- `ProxyKit.kt` 无测试（README B 节已标注，Spring 版本升级时的回归风险点）。
- `ComponentInitializationDispatcher.kt` 无测试：`@AutoConfigureAfter/Before` → dependsOn 翻译、before/afterInit 时序、以及**本轮修复的 early-reference 幂等性**都需要集成测试钉住。
- `CustomConstraintValidatorFactory.kt` / `IConstraintValidatorProviderBean` 装配路径无测试。
- `AbstractFailedDataHandler.kt` 的 persist → read 序列化往返无测试（含本轮新增的 dataType lazy 缓存）。
- `OrderProperties.putIfAbsent` / 两参 `remove`（本轮新增覆写）待补用例。

### 5. 可扩展性

- `src/io/kudos/context/init/ComponentInitializerSelector.kt`：扫描包硬编码 `io.kudos.context|ability|ms`（README 已知约束 4）。建议 `@EnableKudos` 增加 `basePackages: Array<String>` 属性合并扫描。
- `src/io/kudos/context/init/ComponentInitializerSelector.kt`：`getExclusionComponentInitializer` 直接遍历主类 `annotations`，当 `@EnableKudos` 作为 meta-annotation 组合进自定义注解时（其 `@Target` 明确允许 ANNOTATION_CLASS）读不到 `exclusions`。建议改用 `importingClassMetadata.getAnnotationAttributes(EnableKudos::class.java.name)`（Spring 自带 meta-annotation 解析）。
- `src/io/kudos/context/config/OrderProperties.kt`：`compute` / `computeIfAbsent` / `computeIfPresent` / `merge` 仍绕过 `keyList`（本轮只补了最常用的 `putIfAbsent` / 两参 `remove`）。若要求完整 Map 契约需全量覆写，配置加载场景目前不会触达这些方法。

### 6. 可观测性

- 锁框架（`NormalLockService` / `LockTool`）无任何指标埋点：获取失败率、持锁时长、过期清理量是分布式锁健康度的关键信号，建议预留 Micrometer 钩子（可选依赖）。
- `FailedDataRetryScanner.retry`：重试成功/失败只有逐文件日志，无累计计数（如 `kudos.retry.success/fail` 计数器），积压量（目录文件数）也值得作为 gauge 暴露。
- `ComponentInitializationDispatcher`：组件初始化只有 banner 日志，无每组件耗时统计，启动慢时难定位；建议 beforeInit→afterInit 记录耗时并汇总输出。

### 7. 可维护性

- `src/io/kudos/context/lock/NormalLockService.kt`：教学式 KDoc 与代码比例约 2:1（如 `getDelay` 的 20 行注释），信息密度低、改代码时易与注释脱节，建议精简为契约级注释。
- `src/io/kudos/context/support/Consts.kt`：`RequestHeader` 命名风格混杂（`_sub_sys_code` 小写下划线 vs `_UUID`/`_LOCAL`/`_DATA_SOURCE_ID` 大写），属跨进程 wire format 不能直接改名，建议在 v2 协议统一并提供兼容期双写。
- `src/io/kudos/context/init/ComponentInitializerSelector.kt`：`selectImports` 里 `it.qualifiedName ?: it.simpleName ?: "Unknown"` 的 "Unknown" 兜底值若真出现会让 Spring import 失败，应直接过滤掉无 qualifiedName 的类并告警。

### 8. 对外 API 稳定性 / 简洁性

- `src/io/kudos/context/lock/LockTool.kt`：4 个 `lockExecute` 重载全是对 `ILeaseLockProvider.lockExecute` 默认实现的转发，加上已 `@Deprecated` 的 `hasKeyLock`，API 面偏宽；建议下个大版本收敛为"默认租约时长常量 + 2 个重载"。
- `src/io/kudos/context/lock/ILockProvider.kt`：`IReentrantLockProvider.unLock(lock, key)` 的 `lock` 参数对本地实现无用、对 Redisson 必需，契约含糊（KDoc 已说明）；建议 v2 引入 `LockHandle` 抽象统一释放语义。
- `src/io/kudos/context/validation/IConstraintValidatorProviderBean.kt`：`fun <T: Annotation, V: ConstraintValidator<T, *>> provide(): Map<KClass<T>, KClass<V>>` 的泛型由调用方任意实化，是伪类型安全（工厂调用处直接上界擦除）。更诚实的签名是 `Map<KClass<out Annotation>, KClass<out ConstraintValidator<*, *>>>`，属破坏性变更留待大版本。

### 9. 文档

- README 与 KDoc 整体充分（含 8-dim 审计补遗）。补充建议：`@EnableKudos` 文档应注明当前 meta-annotation 组合用法下 `exclusions` 不生效的限制（见上面第 5 节第 2 条），避免使用方踩坑。
