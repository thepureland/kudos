dependencies {
    ...
    // SeataXidPropagationAutoConfiguration needs both type sets to compile, but it only activates when the
    // classpath has both at runtime (guarded by @ConditionalOnClass), so we use compileOnly to avoid
    // leaking interface-client / spring-web dependencies downstream.
    ...
    // main app's interface-client calls fail with "Load balancer does not contain an instance for the service ms12".
    // Adding starter-web to the test classpath lets the sub-apps start a real servlet container -> register
    // -> the LoadBalancer can locate the instances.
    //
