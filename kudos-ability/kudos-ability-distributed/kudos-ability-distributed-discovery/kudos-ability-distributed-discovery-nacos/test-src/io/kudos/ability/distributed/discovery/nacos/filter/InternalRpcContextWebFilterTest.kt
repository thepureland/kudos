...
/**
 * Unit tests for the inbound context-restoration behavior of [InternalRpcContextWebFilter].
 *
 * Coverage:
 *  - requests without the RPC_REQUEST / NOTIFY_REQUEST marker pass through untouched
 ...
 *  - blank (whitespace-only) RPC_REQUEST / NOTIFY_REQUEST markers are treated as unmarked
 *  - blank (whitespace-only) LOCAL header is ignored
 *  - blank DATASOURCE_ID header does not overwrite the context datasource
 *  - IRpcProviderContextProcess SPI is invoked; a throwing processor does not break the chain
 ...
 * @since 1.0.0
 */
internal class InternalRpcContextWebFilterTest {
    ...
    fun doFilter_withoutFeignOrNotifyMarker_doesNotModifyContext() {
        ...
        InternalRpcContextWebFilter().doFilter(request, MockHttpServletResponse(), MockFilterChain())
    ...
    fun doFilter_blankMarkers_treatedAsUnmarked() {
        ...
        InternalRpcContextWebFilter().doFilter(request, MockHttpServletResponse(), chain)
    ...
    fun doFilter_allowUnmarkedContextHeaders_writesHeadersWithoutMarker() {
        ...
        InternalRpcContextWebFilter(allowUnmarkedContextHeaders = true)
    ...
    fun doFilter_nonHttpServletRequest_passesThroughWithoutContextChange() {
        ...
        InternalRpcContextWebFilter().doFilter(raw, MockHttpServletResponse(), chain)
    ...
    fun doFilter_legacyUnsignedMode_acceptsRepeatedRequests() {
        ...
        val filter = InternalRpcContextWebFilter()
        repeat(2) { i ->
            KudosContextHolder.clear()
            val request = MockHttpServletRequest().apply {
                addHeader(Consts.RequestHeader.RPC_REQUEST, "true")
     ...
     * @since 1.0.0
     */
    private class RecordingProviderContextProcess : IRpcProviderContextProcess {
        var calls = 0
     ...
     * @since 1.0.0
     */
    private class ThrowingProviderContextProcess : IRpcProviderContextProcess {
        override fun processContext(request: HttpServletRequest, context: KudosContext) {
