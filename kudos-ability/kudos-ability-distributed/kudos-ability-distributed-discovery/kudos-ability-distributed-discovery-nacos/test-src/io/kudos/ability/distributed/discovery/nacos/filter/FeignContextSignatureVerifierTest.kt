...
/**
 * Direct unit tests for [FeignContextSignatureVerifier].
 *
 * Coverage:
 ...
 * @since 1.0.0
 */
internal class FeignContextSignatureVerifierTest {

    @Test
    fun constructor_blankSecret_throws() {
        val e = assertFailsWith<IllegalArgumentException> {
            FeignContextSignatureVerifier(secret = "   ")
        }
    ...
    fun constructor_nonPositiveTimestampWindow_throws() {
        val zero = assertFailsWith<IllegalArgumentException> {
            FeignContextSignatureVerifier(secret = SECRET, timestampWindowMillis = 0)
        }
        assertTrue(zero.message!!.contains("timestampWindowMillis must be positive"))
        assertFailsWith<IllegalArgumentException> {
            FeignContextSignatureVerifier(secret = SECRET, timestampWindowMillis = -1)
        }
    }
    ...
    fun constructor_nonPositiveNonceCacheMaxSize_throws() {
        val e = assertFailsWith<IllegalArgumentException> {
            FeignContextSignatureVerifier(secret = SECRET, nonceCacheMaxSize = 0)
        }
    ...
    fun verify_missingOrBlankSignatureHeaders_returnsMissing() {
        ...
        assertEquals(FeignContextSignatureVerifier.Result.MISSING_SIGNATURE_HEADERS, verifier.verify(bare))

        // Blank timestamp
        val blankTimestamp = signedRequest(nonce = "n1").apply {
            removeHeader(FeignContextSignatureVerifier.TIMESTAMP_HEADER)
            addHeader(FeignContextSignatureVerifier.TIMESTAMP_HEADER, " ")
        }
        assertEquals(
            FeignContextSignatureVerifier.Result.MISSING_SIGNATURE_HEADERS,
            verifier.verify(blankTimestamp)
        )

        // Blank nonce
        val blankNonce = signedRequest(nonce = "n2").apply {
            removeHeader(FeignContextSignatureVerifier.NONCE_HEADER)
            addHeader(FeignContextSignatureVerifier.NONCE_HEADER, "")
        }
        assertEquals(FeignContextSignatureVerifier.Result.MISSING_SIGNATURE_HEADERS, verifier.verify(blankNonce))

        // Blank signature
        val blankSignature = signedRequest(nonce = "n3").apply {
            removeHeader(FeignContextSignatureVerifier.SIGNATURE_HEADER)
            addHeader(FeignContextSignatureVerifier.SIGNATURE_HEADER, "")
        }
        assertEquals(
            FeignContextSignatureVerifier.Result.MISSING_SIGNATURE_HEADERS,
    ...
    fun verify_nonceHeaderAbsent_whileTimestampPresent_returnsMissing() {
        ...
        assertEquals(FeignContextSignatureVerifier.Result.MISSING_SIGNATURE_HEADERS, verifier.verify(request))
    }
    ...
    fun verify_nonNumericTimestamp_returnsInvalidTimestamp() {
        ...
        assertEquals(FeignContextSignatureVerifier.Result.INVALID_TIMESTAMP, verifier.verify(request))
    }

    @Test
    fun verify_timestampOutOfWindow_pastAndFuture_returnsOutOfWindow() {
        val verifier = fixedVerifier()
        val window = FeignContextSignatureVerifier.DEFAULT_TIMESTAMP_WINDOW_MILLIS

        val tooOld = signedRequest(timestamp = (NOW - window - 1).toString(), nonce = "n-old")
        assertEquals(FeignContextSignatureVerifier.Result.TIMESTAMP_OUT_OF_WINDOW, verifier.verify(tooOld))

        val tooNew = signedRequest(timestamp = (NOW + window + 1).toString(), nonce = "n-new")
        assertEquals(FeignContextSignatureVerifier.Result.TIMESTAMP_OUT_OF_WINDOW, verifier.verify(tooNew))
    }
    ...
    fun verify_timestampExactlyOnWindowBoundary_accepted() {
        ...
        assertEquals(FeignContextSignatureVerifier.Result.OK, verifier.verify(request))
    }
    ...
    fun verify_wrongSignature_returnsSignatureMismatch() {
        ...
        val request = signedRequest(nonce = "n-bad").apply {
            removeHeader(FeignContextSignatureVerifier.SIGNATURE_HEADER)
            addHeader(FeignContextSignatureVerifier.SIGNATURE_HEADER, "definitely-not-the-signature")
        }
        assertEquals(FeignContextSignatureVerifier.Result.SIGNATURE_MISMATCH, verifier.verify(request))
    }
    ...
    fun verify_nullMethodAndRequestUri_treatedAsEmptyStrings() {
        ...
        val request = MockHttpServletRequest(null, null).apply {
            addHeader(FeignContextSignatureVerifier.TIMESTAMP_HEADER, NOW.toString())
            addHeader(FeignContextSignatureVerifier.NONCE_HEADER, nonce)
            addHeader(FeignContextSignatureVerifier.SIGNATURE_HEADER, hmacSha256(payload, SECRET))
        }
        assertEquals(FeignContextSignatureVerifier.Result.OK, verifier.verify(request))
    }
    ...
    fun verify_concurrentSameNonce_exactlyOneAccepted() {
        ...
        val results = ConcurrentLinkedQueue<FeignContextSignatureVerifier.Result>()
        ...
        assertEquals(
            1,
            results.count { it == FeignContextSignatureVerifier.Result.OK },
            "exactly one concurrent request with the same nonce may be accepted"
        )
        assertEquals(
            threadCount - 1,
            results.count { it == FeignContextSignatureVerifier.Result.NONCE_REPLAYED }
        )
    }
    ...
    fun verify_replayedNonce_returnsNonceReplayed() {
        val verifier = fixedVerifier()
        assertEquals(FeignContextSignatureVerifier.Result.OK, verifier.verify(signedRequest(nonce = "n-replay")))
        assertEquals(
            FeignContextSignatureVerifier.Result.NONCE_REPLAYED,
    ...
    fun verify_nonceCacheEviction_oldestNonceAcceptedAgainAfterCapExceeded() {
        ...
        assertEquals(FeignContextSignatureVerifier.Result.OK, verifier.verify(signedRequest(nonce = "n-a")))
        assertEquals(FeignContextSignatureVerifier.Result.OK, verifier.verify(signedRequest(nonce = "n-b")))
        // Third unique nonce evicts the oldest ("n-a") because the cap is 2
        assertEquals(FeignContextSignatureVerifier.Result.OK, verifier.verify(signedRequest(nonce = "n-c")))
    ...
    fun verify_expiredNoncesPurged_notTreatedAsReplayAfterWindowPassed() {
        ...
        assertEquals(
            FeignContextSignatureVerifier.Result.OK,
    ...
    private fun fixedVerifier() = FeignContextSignatureVerifier(
        secret = SECRET,
        clock = Clock.fixed(Instant.ofEpochMilli(NOW), ZoneOffset.UTC)
    )
