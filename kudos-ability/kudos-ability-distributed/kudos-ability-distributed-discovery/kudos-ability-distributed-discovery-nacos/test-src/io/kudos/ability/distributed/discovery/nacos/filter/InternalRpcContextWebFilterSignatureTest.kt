...
/**
 * Unit tests for HMAC signature enforcement in [InternalRpcContextWebFilter] +
 ...
 * contract (module `kudos-ability-distributed-client-http`): HMAC-SHA256 (Base64) over
 ...
 * @since 1.0.0
 */
internal class InternalRpcContextWebFilterSignatureTest {
    ...
    fun expiredTimestamp_rejectedWith401() {
        val filter = filterWithVerifier()
        val expired = NOW_MILLIS - InternalRpcContextSignatureVerifier.DEFAULT_TIMESTAMP_WINDOW_MILLIS - 1
    ...
    private fun filterWithVerifier(): InternalRpcContextWebFilter {
        val verifier = InternalRpcContextSignatureVerifier(
            secret = SECRET,
            clock = Clock.fixed(Instant.ofEpochMilli(NOW_MILLIS), ZoneOffset.UTC)
        )
        return InternalRpcContextWebFilter(signatureVerifier = verifier)
    }
    ...
    private fun signedRequest(
        ...
        return MockHttpServletRequest("POST", REQUEST_URI).apply {
            ...
            addHeader(InternalRpcContextSignatureVerifier.SIGNATURE_HEADER, hmacSha256(payload))
        }
    }
