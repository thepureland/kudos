...
import io.kudos.ability.distributed.discovery.nacos.support.IRpcProviderContextProcess
 ...
 *
 * Counterpart: `KudosContextRequestInterceptor` in `kudos-ability-distributed-client-http` writes
 ...
 * as `FilterRegistrationBean<InternalRpcContextWebFilter>`** — enabled by default, can be disabled with
 * `kudos.ability.distributed.discovery.nacos.rpc-context-filter.enabled=false`.
 *
 ...
 * 3. Extension support: allows extending the context handling via the IRpcProviderContextProcess SPI
 *
 ...
 *
 * Workflow:
 * - Check whether the request carries a Feign / notification propagation marker ([Consts.RequestHeader.RPC_REQUEST] or [Consts.RequestHeader.NOTIFY_REQUEST] non-empty)
 ...
 * - When a [InternalRpcContextSignatureVerifier] is supplied (i.e. the provider configures
 ...
 *   [InternalRpcContextSignatureVerifier]. Requests that fail (including requests missing the headers)
 ...
 * @since 1.0.0
 */
class InternalRpcContextWebFilter(
    private val allowUnmarkedContextHeaders: Boolean = false,
    private val signatureVerifier: InternalRpcContextSignatureVerifier? = null
) : Filter {
     ...
     *
     * Request gate:
     * - Only parse context when either the RPC_REQUEST or NOTIFY_REQUEST header is non-empty (both are explicit propagation markers)
     *
     ...
     *
     * Extension handling:
     * - Invoke all IRpcProviderContextProcess implementations for additional context processing
    ...
    override fun doFilter(
        ...
        val isFeign = !request.getHeader(Consts.RequestHeader.RPC_REQUEST).isNullOrBlank()
        ...
        SpringKit.getBeansOfType<IRpcProviderContextProcess>().values.forEach { processor ->
            try {
                processor.processContext(request, context)
            } catch (e: Exception) {
                log.error(e, "IRpcProviderContextProcess execution failed: {0}", processor.javaClass.name)
            }
        }
        filterChain.doFilter(servletRequest, servletResponse)
    }
    ...
    private fun verifySignature(request: HttpServletRequest, servletResponse: ServletResponse?): Boolean {
        ...
        if (result == InternalRpcContextSignatureVerifier.Result.OK) {
            return true
        }
