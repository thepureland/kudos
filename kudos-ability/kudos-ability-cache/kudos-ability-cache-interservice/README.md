...
跨服务（inter-service）缓存协作三件套。在跨服务调用链上实现类似 HTTP ETag /
...
| [`kudos-ability-cache-interservice-client`](kudos-ability-cache-interservice-client/README.md) | client 端：`HttpCacheNegotiationInterceptor`（请求 + 响应两侧合一）+ 本地缓存 helper |
