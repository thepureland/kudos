...
internal class RateLimitingWebSocketHandlerTest {
    ...
    @Test
    fun burst_isAllowed_thenFurtherMessagesAreDropped(): Unit = runBlocking {
    ...
    @Test
    fun tokensRefillOverTime(): Unit = runBlocking {
        val delegate = RecordingHandler()
    ...
    @Test
    fun refillIsCappedAtBurst(): Unit = runBlocking {
        val delegate = RecordingHandler()
        val handler = limiter(delegate, permitsPerSecond = 10.0, burst = 2)
        val session = StubSession("s1")
    ...
    @Test
    fun eachSessionGetsItsOwnBudget(): Unit = runBlocking {
        val delegate = RecordingHandler()
    ...
    @Test
    fun textAndBinaryShareTheSameBudget(): Unit = runBlocking {
        val delegate = RecordingHandler()
        val handler = limiter(delegate, burst = 2)
        val session = StubSession("s1")
    ...
    @Test
    fun onLimitExceeded_canBeOverriddenToCloseTheSession(): Unit = runBlocking {
        val delegate = RecordingHandler()
        val session = StubSession("s1")
        val handler = object : RateLimitingWebSocketHandler(delegate, 10.0, 1, clock::get) {
            override suspend fun onLimitExceeded(session: KudosWebSocketSessionRef) {
                session.close(WebSocketCloseReason(WebSocketCloseReason.Codes.VIOLATED_POLICY, "too chatty"))
            }
        }
        ...
        assertEquals(WebSocketCloseReason.Codes.VIOLATED_POLICY.code, session.closeReason?.code,
